...
结合当前 phase 1 的协议约束，推荐的轮换策略是：

- 空闲时接近时限，可以直接断开并立刻重连
- 如果当前正在处理一条消息，不建议处理中途强制切流
- 更稳妥的方式是等当前消息 `complete` 之后再建新流

这是因为当前第一阶段还没有：

- `delivery_id`
- `client_instance_id`
- 显式 ack / redelivery 协议
...
如果平台超时是稳定存在的，还建议服务端补一个基础兜底：

- inflight 超时回收机制

它的用途是：

- 避免某条消息已经从 `queued` 移到 `inflight`
- 但 worker 因连接轮换、进程异常、机器故障等原因没有 `complete`
- 导致该消息永久卡住
