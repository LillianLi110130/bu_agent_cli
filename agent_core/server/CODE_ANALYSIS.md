...
```bash
uvicorn agent_core.server.example_server:app --reload --port 8000
```

### 7.2 生产环境

```bash
uvicorn agent_core.server.example_server:app \
    --host 0.0.0.0 \
    --port 8000 \
    --workers 4 \
    --log-level info
...
WORKDIR /app
RUN pip install -e .
CMD ["uvicorn", "agent_core.server.example_server:app", "--host", "0.0.0.0"]
```
