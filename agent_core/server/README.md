...
FastAPI-based HTTP server for the agent_core, enabling remote invocation of agent capabilities via REST API.
...
```python
from agent_core.server import ServerConfig, create_app
...
CMD ["uvicorn", "agent_core.server.example_server:app", "--host", "0.0.0.0", "--port", "8000"]
```
