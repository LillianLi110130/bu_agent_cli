"""
FastAPI 应用 - agent_core HTTP 服务器

这是 agent_core 的 HTTP API 主文件，提供以下功能：
1. 会话管理：创建、查询、删除会话
2. Agent 查询：非流式和流式查询
...
logger = logging.getLogger("agent_core.server")
...
def _agent_event_to_stream_event(event) -> StreamEvent:
    ...
    if isinstance(event, AgentTextDeltaEvent):
        from agent_core.server.models import TextDeltaEvent
...
def create_app(
    ...
    app = FastAPI(
        ...
        description="HTTP API for agent_core - Agentic applications with LLMs",
        version="1.0.0",
    ...
    async def _resolve_query_session(request: QueryRequest):
        ...
        if (
            ...
            and existing_session.user_id is None
    ...
    async def list_skills():
        ...
        返回系统中所有已加载的技能列表。
        """
        from agent_core.server.skills import get_global_registry
