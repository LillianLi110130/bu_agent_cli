# agent_core.agent 代码结构文档

## 概述

`agent_core.agent` 模块实现了一个简单的 Agentic 循环，支持原生工具调用（tool calling）和自动上下文压缩。该模块采用流式事件驱动的设计模式，提供对 Agent 执行过程的细粒度可见性。
