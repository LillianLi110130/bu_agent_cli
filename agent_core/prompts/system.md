...
- 若要基于当前上下文 fork 一个子执行体，不要传 `subagent_type`。fork 适合短期复用当前上下文的局部执行，不适合再做二次分发。
...
- 前台 / 后台选择规则：
  ...
  - 只有当任务较长且当前主流程不需要立刻消费最终结果时，才设置 `run_in_background=true`
  - `delegate_parallel(...)` 只用于前台并行，不用于后台任务
- 后台 subagent 启动后，不要假设它已经完成。后续如需查看结果，应使用 `/task <task_id>` 或 `/tasks` 查看状态与结果。
