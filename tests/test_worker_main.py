...
def test_async_main_can_read_auth_config_from_source_dir(monkeypatch):
    root = Path(".pytest_tmp") / f"worker-main-source-{uuid.uuid4().hex}"
    if root.exists():
        shutil.rmtree(root)
    config_dir = root / "state"
    config_source_dir = root / "packaged"
    config_dir.mkdir(parents=True)
