...
- `kb/raw/` 支持按主题、来源或时间继续细分子目录；子目录层级不限，记录来源时使用相对于工作区的完整路径
...
- 页面内涉及 `相关概念`、`相关实体`、`关键来源` 等栏目时，只要本次编译已经确定目标页及其路径，就默认写入标准 Markdown 相对链接；相对路径必须以当前页面所在目录为基准计算，不要套用其他页面模板中的路径写法。
...
- 当编译时没有指定来源路径，应递归检查 `kb/raw/` 及其子目录中哪些来源还没有出现在这个文件里
- 每当一个来源成功编译完成后，都应立即把它补充到这个文件里

建议使用统一条目格式：

`- source: kb/raw/topic/example.md`
...
- 同一来源再次编译时，应明确说明“本次是二次编译”或“本次是补充编译”，并简要写明主要补充或修正点
- 提到摘要页、概念综合页、实体页或综合页时，只要本次编译已经能够确定页面及其路径，就应使用标准 Markdown 相对链接
- 提到 `kb/raw/` 下的原始来源时，使用纯路径文本，不使用 Markdown 链接
...
如果没有指定来源路径，就把 `kb/raw/` 及其子目录中尚未登记为已编译的来源逐个编译。
