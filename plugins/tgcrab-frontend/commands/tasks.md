---
name: tasks
description: 使用当前 agent 执行前端任务拆解并生成 Ralph 计划
usage: /tgcrab-frontend:tasks <spec_name>
category: Frontend
examples:
...
- `spec_name` 为：`{{args}}`
- 本阶段的主要输入是：
  - `docs/spec/{{args}}/artifacts/tgcrab-frontend/01_requirement.md`
  - `docs/spec/{{args}}/artifacts/tgcrab-frontend/02_design.md`

## 核心目标

1. 完整理解需求分析和技术设计文档。
2. 将需求进一步细化为可独立实现、可独立验证、边界清晰的原子任务。
3. 为每个任务生成一份 `docs/spec/{{args}}/plan/*.md` 规划文档。
4. 生成结构正确、可直接被 Ralph Loop 消费的 `docs/spec/{{args}}/plan/plan.json`。

## 执行步骤

1. 同时读取 `docs/spec/{{args}}/artifacts/tgcrab-frontend/01_requirement.md` 与 `docs/spec/{{args}}/artifacts/tgcrab-frontend/02_design.md`。
2. 基于需求和设计直接生成最终计划。
3. 直接生成 `docs/spec/{{args}}/plan/plan.json` 和 `docs/spec/{{args}}/plan/*.md`。
...
`docs/spec/{{args}}/plan/plan.json` 必须是一个合法的 JSON 数组。每个任务对象必须包含以下字段：
...
1. `docs/spec/{{args}}/plan/plan.json` 是合法 JSON，且可被标准 JSON 解析器解析
2. `docs/spec/{{args}}/plan/plan.json` 中每个任务的 `task_file` 都存在
