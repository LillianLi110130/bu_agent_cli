...
- 结论尽量绑定具体摘要页和原始来源路径
- `llm-wiki/wiki/index.md` 是内容导航入口，回答前优先读取
- `llm-wiki/wiki/log.md` 记录整个知识库的演化时间线
...
- 页面内涉及 `相关概念`、`相关实体`、`关键来源` 等栏目时，若目标页已存在，默认使用标准 Markdown 相对链接。
- 如果目标页尚不存在，不要伪造链接；应明确标注为“待补页面”或“待补概念”

## 索引维护规则

每次将新来源编译进 Wiki，或进行了有意义的结构性维护后，都应更新 `llm-wiki/wiki/index.md`。
...
如果日志中提到已经存在的摘要页、概念综合页或实体页，尽量使用标准 Markdown 相对链接，而不是只写纯文本名称。
如果日志中提到已经存在的综合页，也尽量使用标准 Markdown 相对链接，而不是只写纯文本名称。
...
3. 不足时再回退原始来源
4. 给出带引用的回答
5. 如果本次问题产出了高价值综合结论、比较、分析框架或新连接，应建议沉淀到正式 wiki 页面
6. 除非用户明确要求写回 wiki，否则默认只回答并给出沉淀建议，不直接修改页面

查询结果的落页原则：

- 如果只是补充已有知识，优先更新现有概念综合页或实体页
- 如果形成了新的跨来源比较、专题分析、重要连接或阶段性综合结论，可新建 `llm-wiki/wiki/synthesis/` 下的综合页
...
source_path: llm-wiki/raw/example.md
source_kind: article
---
...
- 只有在 wiki 本身不足时，才回退读取 `llm-wiki/raw/`
...
def _home_content() -> str:
    return (
        ...
        + "## 使用方式\n\n"
        + "- 将原始资料放入 `llm-wiki/raw/`\n"
        + "- 运行 `/wiki:ingest <source-path>` 将资料逐步编译进 Wiki\n"
        + "- 运行 `/wiki:query <question>` 基于现有 Wiki 回答问题\n"
        + "- 运行 `/wiki:lint` 定期检查矛盾、缺口与孤儿页\n\n"
        + "## 维护原则\n\n"
        + "- 原始资料只读\n"
        + "- 正式知识沉淀在 `llm-wiki/wiki/`\n"
        + "- 页面之间应主动建立交叉链接\n"
    )
...
def main() -> int:
    ...
    wiki_root = working_dir / "llm-wiki"
    ...
    if kept_files:
        ...
        for item in kept_files:
            print(f"  - {item}")
    print("下一步建议：")
    print("  1. 把资料放进 llm-wiki/raw/")
    print("  2. 运行 /wiki:ingest <source-path>")
    print("  3. 运行 /wiki:query <question>")
    return 0
