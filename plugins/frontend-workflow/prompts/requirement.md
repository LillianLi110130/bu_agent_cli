...
1. **读取输入**: 先完整读取 `{{input_dir}}` 下的原始材料。
2. **需求性质与自检**:
   - 判断本次任务是 `[feature]`、`[refactor]` 还是 `[fix]`
   - 若需求模糊，必须在文档中写出“待澄清问题”，禁止在不确定的情况下凭空补需求
3. **执行需求分析**: 基于输入材料整理需求背景、目标、范围、关键流程和验收标准。
4. **保存文档**: 将结果保存到 `{{requirement_file}}`。
