...
def build_init_system_prompt() -> str:
    ...
    return (
        ...
        "When a tool result says `Context preview`, `trimmed`, `summary`, or `Artifact file`, "
        "treat it as a context-limited preview rather than the full original content.\n"
        ...
        "or `n_lines`; if the result explicitly points to an artifact file, read that artifact "
        "with explicit `offset_line` and `n_lines` instead of repeating the same source read.\n"
...
def build_init_user_prompt(workspace_root: Path) -> str:
    ...
    return (
        ...
        "- If a tool result shows `Context preview`, `trimmed`, `summary`, or `Artifact file`, "
        "treat it as a preview, not the full original content.\n"
        "- Do not repeat an identical `read`, `glob_search`, `grep`, or `resolve_path` call just "
        "to see more; change the range/query, or read the referenced artifact with explicit "
        "window parameters when necessary.\n"
