...
OutboundAction = Literal["text", "attachment"]
...
@dataclass
class BridgeOutboundEvent:
    """One queued outbound message generated by local tools."""
    ...
    def to_dict(self) -> dict[str, Any]:
        """Serialize the outbound event for JSON storage."""
        payload = asdict(self)
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "BridgeOutboundEvent":
        """Deserialize an outbound event from JSON payload."""
        return cls(
            version=int(payload["version"]),
